...
class PenjualanDetailController extends Controller
{
    ...
    public function listData($id)
    {
        ...
        foreach($detail as $list) {
            ...
            $row[] = "<input type='number' class='form-control' name='jumlah_$list->id_penjualan_detail' 
                      value='$list->jumlah' onChange='changeCount($list->id_penjualan_detail)'>";
            $row[] = $list-> diskon."%";
            $row[] = "Rp. ".format_uang($list-> sub_total);      
            $row[] = '<div class="btn-group">
                        <a onclick = "deleteItem('.$list -> id_penjualan_detail.')" class="btn btn-danger btn-sm">
        ...
        $data[] = array(
                        "<span class='hide total'>$total</span><span class='hide totalitem'>$total_item</span>"
                        ,"","","","","","",""
                        );
    ...
    public function store(Request $request)
    {
        ...
        //sub_total disesuakan dengan diskon
    ...
    public function saveData(Request $request)
    {
        ...
        //update stock data sesuai data yang dibeli
        $detail = PenjualanDetail :: where('id_penjualan', '=', $request['idpenjualan'])
                -> get();
        foreach ($detail as $data) {
            $produk = Produk :: where('kode_produk', '=', $data-> kode_produk)
                    -> first();
    ...
    public function printNota()
    {
        ...
        if ($setting -> tipe_nota == 0) {
            ...
            $font = printer_create_font("Consolas", 72, 48, 400, false, false, flase, 0);
            printer_select_font($handle, $font);
    ...
    {
        $detail    = PenjualanDetail :: leftJoin('produk', 'produk.kode_produk', '=', 'penjualan_detail.produk')
            -> where('id_penjualan', '=', session('idpenjualan'))
            -> get();
